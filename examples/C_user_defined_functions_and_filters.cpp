...
void cross_alert_example() {
  ...
  InputStream<TimeValue> ts =
      NewInputStream<TimeValue>("TS", true /* verbose */);
  Streamulus engine;
  ...
  // print and cross_alert are streamified from the types (this work because
  // they can be default constructed).

  // Print the moving averages:
  engine.Subscribe(Streamify<print>(std::string("Slow Mavg = ") +
                                    Streamify<as_string>(slow)));
  engine.Subscribe(Streamify<print>(std::string("Fast Mavg = ") +
                                    Streamify<as_string>(fast)));

  // The cross detection expression:
  engine.Subscribe(
      Streamify<cross_alert>(Streamify<unique<bool>>(slow < fast)));
