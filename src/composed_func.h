...
template <typename Signature> class ComposedFunc;
...
public:
  ...
  typename boost::optional<
      typename result<ComposedFunc<F(G(ArgType))>(Arg)>::type>
  operator()(const Arg &arg) {
    boost::optional<
        typename result<ComposedFunc<F(G(ArgType))>(Arg)>::G_result_type>
        g_res = g(arg);
    if (!g_res)
      return boost::none;
    boost::optional<
        typename result<ComposedFunc<F(G(ArgType))>(Arg)>::F_result_type>
        f_res = f(*g_res);
    if (!f_res)
      return boost::none;
    return *f_res;
