...
public:
  Engine()
      : mWorking(false), mCurrentTime(0), mVerbose(false),
  ...
  void AddSource(const StropPtr &strop) { mSources.push_back(strop); }

  template <typename StreamPtrType>
  void AddEdgeToGraph(const StropPtr &source, const StropPtr &target,
                      const StreamPtrType &stream) {
    StreamPtr stream_ptr(stream);
    Graph::edge_descriptor desc =
        boost::add_edge(source->GetDescriptor(), target->GetDescriptor(),
                        stream_ptr, mGraph)
            .first;
    if (stream->IsValid())
  ...
  void Output(const BoostGraph::vertex_descriptor &source, const T &value) {
    ...
    for (boost::tie(it, it_end) = boost::out_edges(source, mGraph);
         it != it_end; ++it) {
  ...
  void Work() {
    ...
    // std::cout << "Work called. mQueue.size() = " << mQueue.size() <<
    // std::endl;
    while (!mQueue.empty()) {
...
private:
  ...
  void ActivateSources() {
    if (IsVerbose())
      std::cout << "Activate sources: mSources.size() = " << mSources.size()
                << std::endl;
    for (std::vector<StropPtr>::iterator it = mSources.begin();
         it != mSources.end(); ++it)
      ActivateVertex(*it);
    mSources.clear();
  }
  ...
  bool IsRemoveable(const StropPtr &strop) {
    ...
    for (boost::tie(it, it_end) =
             boost::out_edges(strop->GetDescriptor(), mGraph);
         it != it_end; ++it) {
      BoostGraph::edge_descriptor edge(*it);
  ...
  void RemoveVertexFromGraph(const StropPtr &strop) {
    ...
    for (boost::tie(it, it_end) =
             boost::in_edges(strop->GetDescriptor(), mGraph);
         it != it_end; ++it) {
      BoostGraph::edge_descriptor edge(*it);
  ...
  void DoGarbageCollection() {
    ...
    std::map<BoostGraph::vertex_descriptor, BoostGraph::vertex_descriptor>
        descriptors_old2new;

    // copy live nodes
    BoostGraph::vertex_iterator vit, vit_end;
    for (boost::tie(vit, vit_end) = boost::vertices(mGraph); vit != vit_end;
         ++vit) {
      const StropPtr &strop(mGraph[*vit]);
      if (!strop->IsDeleted())
        descriptors_old2new[strop->GetDescriptor()] =
            boost::add_vertex(strop, new_graph);
    }

    // copy live edges
    BoostGraph::edge_iterator eit, eit_end;
    for (boost::tie(eit, eit_end) = boost::edges(mGraph); eit != eit_end;
         ++eit) {
  ...
  struct TopologicalSortVisitor
      : public boost::default_dfs_visitor { // Reset descriptors and recompute
                                            // topological order labels in the
                                            // graph:
  public:
    TopologicalSortVisitor(Graph &g, Engine *engine)
        : mIndex(boost::num_vertices(g)), mGraph(g), mEngine(engine) {}
  ...
  struct QueueEntry {
    QueueEntry(TimestampT time, size_t top_sort_index, StropPtr &strop)
        : mTime(time), mTopSortIndex(top_sort_index), mStrop(strop) {}

    bool operator<(const QueueEntry &rhs) const {
      // sort by (time, topological order)
      return (this->mTime < rhs.mTime) ||
             (this->mTime == rhs.mTime &&
              this->mTopSortIndex < rhs.mTopSortIndex);
    }
  ...
  public:
    ...
    void operator()(std::ostream &out,
                    const Graph::vertex_descriptor &v) const {
      out << "[label=" << mGraph[v]->DisplayName() << "_Desc"
          << mGraph[v]->GetDescriptor() << "_TO" << mGraph[v]->GetTopSortIndex()
          << "]";
    }
