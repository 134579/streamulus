...
template <typename F> class WindowFunc {
public:
  WindowFunc() : mFunction(F()) {}

  WindowFunc(const F &f) : mFunction(f) {}

  template <typename WA>
  boost::optional<typename F::template value_type<
      F(typename WindowBaseType<WA>::type)>::type>
