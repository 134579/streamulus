 ...
 * Current() returns the current value, which is the oldest unread data if there
 * is any, or the youngest read data otherwise.
 */
template <typename T> class Stream : public StreamBase {
public:
  void Append(const T &item) { mBuffer.push_back(item); }

  bool HasMore() { return !mBuffer.empty(); }

  bool IsValid() { return mLastValue || !mBuffer.empty(); }
