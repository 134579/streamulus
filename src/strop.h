...
class Strop : public StropStreamProducer<
                  typename boost::function_types::result_type<F>::type> {

private:
  template <typename T> struct MakeStreamPtrType {
  ...
  using input_types =
      typename boost::mpl::transform<param_types,
                                     MakeStreamPtrType<boost::mpl::_1>>::type;

public:
  virtual ~Strop() {}

  template <typename... Inputs> void SetInputs(Inputs... inputs) {
  ...
  struct extract_input_value {
    ...
    template <typename T> T operator()(std::shared_ptr<Stream<T>> input) const {
  ...
  struct has_more_fold_function {
    template <typename T> bool operator()(const bool &state, const T &t) const {
  ...
  bool IsValid() {
    return NoInputs() ||
           boost::fusion::fold(mInputs, true, is_valid_fold_function());
  }

  bool NoInputs() { return boost::fusion::empty(mInputs); }

private:
  typename boost::fusion::result_of::as_vector<typename input_types::type>::type
      mInputs;
};
