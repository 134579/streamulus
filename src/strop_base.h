...
class StropBase {
public:
  StropBase() { Init("unnamed"); }

  StropBase(const std::string &display_name) { Init(display_name); }
...
public:
  ...
  Graph::vertex_descriptor GetDescriptor() const { return mVertexDescriptor; }

  size_t GetTopSortIndex() const { return mTopSortIndex; }

  Engine *GetEngine() { return mEngine; }

  void SetGraph(Engine *engine, const Graph::vertex_descriptor &desc,
  ...
  bool HasExternalReference() { return mExternalReference; }
...
protected:
  ...
  bool mIsDeleted; // Was deleted from the graph by the user (to be garbage
                   // collected).
  bool mExternalReference; // The user has a handle to this strop (so it can be
                           // used in future expressions).
