...
template <typename R> class DataSource : public StropStreamProducer<R> {
public:
  DataSource(const std::string &name, bool verbose)
      : mIsValid(false), mIsVerbose(verbose) {
  ...
  inline bool IsVerbose() { return mIsVerbose; }

  void Tick(const R &value) {
    if (IsVerbose())
      std::cout << "-------------   " << StropStreamProducer<R>::DisplayName()
                << " <-- " << value << "   -------------" << std::endl;
